...
5. v2/design/decision 문서는 현재 운영 명세가 아닙니다. stable 동작은 루트 운영 문서와 `docs/feature-status.yaml`의 maturity tier를 기준으로 판단합니다.
...
- 사용자가 `goal 프롬프트`를 달라고 하면 이는 Codex CLI의 `/goal` 기능에 넣을 프롬프트를 뜻합니다. 에이전트가 작성하는 `/goal` 프롬프트는 4000자 이하로 유지해야 합니다. 4000자를 넘을 내용이면 상세 범위와 수용 기준을 별도 문서로 정리한 뒤, `/goal` 프롬프트에는 그 문서 경로와 핵심 목표·제약만 짧게 담습니다. 길이 확인이 필요하면 `python3 scripts/goal-prompt-check.py --text "<prompt>"` 또는 stdin을 사용합니다.
...
- 구현 전 비판적 토론이 필요한 작업에서는 Claude 자동 토론을 사용하지 않습니다. 기본 방식은 Codex가 orchestrator로 남고 `subagent-critic`, `subagent-researcher`, `subagent-verifier` 같은 서브 에이전트 의견을 독립적으로 받은 뒤, 열린 block critique가 모두 resolved 또는 accepted_as_risk가 되었을 때만 범위를 고정하는 subagent debate입니다. 기록이 필요하면 `python3 scripts/agent-flow.py dialogue start|add-turn|status|converge`를 사용하고, Claude 세션 만료를 fallback trigger로 삼지 않습니다.
...
- 이미 구버전 뼈대가 적용된 기존 프로젝트를 최신화할 때는 `scripts/release-manifest.py`의 release id/source commit/channel을 기준으로 삼고, `agent-flow adopt` -> `upgrade-from-skeleton --brief --profile stable` -> 승인된 `--apply --safe-only` -> target 검증 순서로 진행합니다. 기존 파일 overwrite, runtime/state/knowledge 정리, incubating/experimental 자동 확산은 금지하고, 적용 결과는 `runtime/install-state.jsonl`의 `skeleton_release_applied` 이벤트와 handoff에 남깁니다.
...
- Notion을 사용하는 프로젝트에서만 Notion 규칙을 적용합니다. 새 페이지나 업데이트 전에는 `docs/NOTION_DOCUMENTATION_RULES.md`의 중복 방지 프로토콜과 기능 문서 품질 기준을 따릅니다.
- 초안 Markdown을 만들 수 있으면 `python3 scripts/notion-doc-quality-check.py <draft.md>`로 검사한 뒤 Notion에 반영합니다.
