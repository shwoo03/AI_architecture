...
| closeout-validator | validator | verify, quality-gate, completion evidence, AgentRun verdict 독립 확인 |
...
전문가에게 위임할 때는 `config/agent-team.yaml`을 canonical registry로 보고, `scripts/agent-brief.py`로 brief를 만든 뒤 전달합니다. brief에는 role, goal, allowed scope, write policy, forbidden actions, recommended checks가 포함되어야 합니다.

프로젝트별 specialist가 필요하면 선택적으로 `config/agent-team-overrides.yaml`을 둡니다. 이 파일은 public command를 늘리거나 generated agent 파일을 만들지 않고, `scripts/agent-brief.py`가 brief 생성 시에만 읽습니다.

Overlay 규칙은 additive-first입니다.

- 새 project specialist는 추가할 수 있으며 brief에 `role_source: project`로 기록됩니다.
- base specialist는 `role_source: base`로 유지됩니다.
- base specialist의 `write_policy`와 `default_scope`는 좁힐 수 있습니다.
- base specialist의 `write_policy`나 `default_scope`를 넓히는 overlay는 오류로 거부됩니다.
- `.codex/agents/`와 `.claude/agents/`는 계속 `agents/`에서 생성되며 specialist overlay의 출력 대상이 아닙니다.

On-demand specialist 흐름은 `scripts/agent-flow.py specialist ...` 하위 명령으로만 노출합니다.
