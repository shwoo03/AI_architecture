# Codemaps

- `generated_at`: 2026-05-16T14:43:52Z
- `root`: /Users/shwoo/mydir/AI/AI_architecture

| area | files | lines | navigation entry points |
| --- | ---: | ---: | --- |
| `agents` | 4 | 96 | - |
| `docs` | 56 | 4707 | `AGENTS.md`, `docs/AGENTS.md` |
| `other` | 129 | 15881 | `.agents/skills/brainstorming/SKILL.md`, `.agents/skills/code-review-expert/SKILL.md`, `.agents/skills/codex-skill-creator/SKILL.md` |
| `reference` | 14 | 1255 | `references.yaml`, `research/AGENTS.md` |
| `rules` | 27 | 1338 | `manifest.yaml` |
| `runtime` | 23 | 6447 | `runtime/AGENTS.md` |
| `scripts` | 85 | 23392 | `scripts/AGENTS.md`, `scripts/agent-flow.py`, `scripts/quality-gate.py` |
| `skills` | 81 | 13224 | `skills/AGENTS.md`, `skills/_meta/project-scaffolder/SKILL.md`, `skills/_meta/reference-refresher/SKILL.md` |
| `tests` | 15 | 8017 | - |

## Usage

Agents should consult this directory before broad file searches when they need a quick map of the harness surface. Agent-callable public commands are still defined only by `scripts/catalog.yaml`.
