...
source anchor를 근거로 실제 채택, 복사, 승격 결정을 내릴 때는 기본 구조 검증에 더해 strict 검증을 사용합니다. 이 검증은 전역 closeout의 기본값이 아니며, reference adoption/proposal/copy/promotion 판단 직전에만 사용합니다.

```bash
python3 scripts/validate-reference-candidates.py --strict-source-anchor
python3 scripts/validate-reference-proposals.py --strict-source-anchor
python3 scripts/reference-copy-ledger.py check --strict-source-anchor
```

strict source anchor는 `checked_revision`, `freshness_signal`, `sources[].hash_or_line_ref` 같은 필드가 `local-reference`, `external-reference`, `not checked` 같은 placeholder에 머물지 않고 재검토 가능한 commit, line, hash, symbol anchor를 갖는지 확인합니다.
