...
## Specialist proposal and delegation plan artifacts
...
session recall은 `runtime/session-recall.sqlite`에 생성되는 재생성 가능한 검색 캐시입니다. 원본은 항상 JSONL, `runtime/state/session-handoff.md`, `state/decisions.md`이며, 캐시가 사라지면 `python3 scripts/session-recall.py index`로 다시 만듭니다. 캐시는 source of truth가 아니며 git에 커밋하지 않습니다.

session recall 캐시는 insert 시점에 secret-like 값을 마스킹합니다. 이미 만들어진 캐시가 이전 redaction 규칙으로 생성됐을 수 있으므로 `recall_meta.index_version`이 현재 코드와 다르면 stale로 보고 재생성합니다. SQLite FTS5 기본 tokenizer는 한국어 형태소 분석을 하지 않으므로 "결정"과 "결정한" 같은 형태 변화는 별도 토큰으로 취급될 수 있습니다.
